...
echo -e "${GREEN}=== Simple Blazecache Load Test ===${NC}\n"
...
# Build and run load test locally (simpler than Docker)
echo -e "${GREEN}Building load test client...${NC}"
cd ..
cargo build --release --example loadtest_100k --manifest-path clients/rust/Cargo.toml

echo -e "${GREEN}Running load test...${NC}\n"
SERVER_ADDR="127.0.0.1:6790,127.0.0.1:6792" \
NUM_OPS=$NUM_OPS \
NUM_WORKERS=$NUM_WORKERS \
./clients/rust/target/release/examples/loadtest_100k
