use blazecache::transports::{TcpClient, ProtocolClient};
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let server_addr = std::env::var("SERVER_ADDR")
        .unwrap_or_else(|_| "127.0.0.1:6784".to_string());
    
    println!("=== Testing TCP Commands ===");
    println!("Server: {}\n", server_addr);
    
    // Connect
    println!("1. Connecting to server...");
    let mut client: TcpClient<BinarySerializer> = match timeout(
    ...
    let test_key = "test-tcp-key";
    let test_value = b"test-tcp-value";
    match timeout(
        Duration::from_secs(5),
    ...
    println!("=== All TCP commands tested successfully! ===");
    Ok(())
}
