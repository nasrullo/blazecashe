...
/// Type alias for an optional peer picker wrapped in async read-write lock.
///
/// Used in the Group struct to store the peer picker that determines which
/// peer should handle each key. Wrapped in Arc and RwLock for thread-safe access.
type PeerPickerHandle = Arc<RwLock<Option<Box<dyn PeerPicker>>>>;
...
pub struct Group {
    ...
    pub(crate) peers: PeerPickerHandle,
...
/// # );
    ...
    pub async fn pick_peer_for_key(&self, key: &str) -> Option<String> {
        if let Some(ref peers) = *self.peers.read().await {
            peers.pick_peer(key).map(|peer| peer.address())
        } else {
            None
        }
    }
    ...
    /// Get aggregated statistics from both main cache and hot cache.
    pub async fn stats(&self) -> CacheStats {
        let main_stats = self.main_cache.stats().await;
        let hot_stats = self.hot_cache.stats().await;
        
        // Aggregate stats from both caches
        CacheStats {
            hits: main_stats.hits + hot_stats.hits,
            misses: main_stats.misses + hot_stats.misses,
            puts: main_stats.puts + hot_stats.puts,
            deletes: main_stats.deletes + hot_stats.deletes,
            evictions: main_stats.evictions + hot_stats.evictions,
            hot_items: main_stats.hot_items + hot_stats.hot_items,
            rejected_items: main_stats.rejected_items + hot_stats.rejected_items,
            ttl_evictions: main_stats.ttl_evictions + hot_stats.ttl_evictions,
            entry_count: main_stats.entry_count + hot_stats.entry_count,
            memory_usage: main_stats.memory_usage + hot_stats.memory_usage,
        }
    }
    ...
    /// 
    /// Note: Holds a read lock across the peer.set() await. This is safe because:
    /// - Read locks allow concurrent readers
    /// - Peer operations are typically fast network calls
    /// - The alternative would require restructuring the Peer trait
    async fn remote_set(&self, key: &str, value: Vec<u8>, ttl: u32) -> Option<Result<()>> {
        ...
        // Read lock held during await is acceptable for read operations
        Some(peer.set(&self.name, key, value, ttl).await)
    }

    /// Send a get to the remote peer responsible for a key (if not local).
    /// 
    /// Note: Holds a read lock across the peer.get() await. This is safe because:
    /// - Read locks allow concurrent readers
    /// - Peer operations are typically fast network calls
    ...
    /// 
    /// Note: Holds a read lock across the peer.delete() await. This is safe because:
    /// - Read locks allow concurrent readers
    /// - Peer operations are typically fast network calls
    ...
    ///
    /// This method removes all cached items from both the main cache and hot cache,
    /// effectively resetting the cache to an empty state. Statistics are preserved
    /// (hits, misses, etc.) but entry_count and memory_usage are reset.
    ///
    /// If peers are configured, this method also forwards the CLEAR command to all
    /// peers in the cluster to ensure consistency across all nodes.
    pub async fn clear(&self) {
        // Clear local caches first
        self.main_cache.clear().await;
        self.hot_cache.clear().await;
        
        // Forward CLEAR to all peers
        self.remote_clear_all().await;
    }

    /// Send CLEAR command to all remote peers in the cluster.
    ///
    /// This method broadcasts the CLEAR command to all peers to ensure
    /// all nodes clear their caches consistently.
    async fn remote_clear_all(&self) {
        let peer_addresses = self.get_peers().await;
        let group_name = self.name.clone();
        
        // Send CLEAR to all peers (excluding self)
        for addr in peer_addresses {
            if self.is_local_peer(&addr) {
                continue; // Skip self
            }
            
            // Create a temporary RemotePeer to send the command
            use crate::networking::remote_peer::RemotePeer;
            use crate::networking::peer::Peer;
            let peer = RemotePeer::new(addr);
            let group_name_clone = group_name.clone();
            
            // Spawn async task to send CLEAR (fire and forget - we don't wait for all)
            tokio::spawn(async move {
                let _ = peer.clear(&group_name_clone).await;
            });
        }
    }
}
...
mod tests {
    ...
    struct MockPeerSet {
        ...
        last_value: Arc<TokioMutex<Option<Vec<u8>>>>,
    }
