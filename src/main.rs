...
use blazecache::transports::{PersistenceManagerHandle, ProtocolServer};
...
use blazecache::transports::{TlsTcpServer, UdpServer};
use std::env;
use std::process::Command;
use std::sync::Arc;
use std::time::Duration;
use tokio::sync::Mutex as AsyncMutex;
use tokio::time::{interval, sleep as tokio_sleep};
...
///
/// Used in the main function and other top-level error handling.
type MainError = Box<dyn StdError + Send + Sync>;
...
async fn main() -> Result<(), MainError> {
    ...
    tokio::spawn(async move {
        if let Err(e) = tcp_server.start(tcp_port).await {
            ...
            if error_msg.contains("Address already in use") || error_msg.contains("os error 98") {
                error!(
                    port = tcp_port,
                    error = %e,
                    "TCP server failed: Port {} is already in use. Please stop the existing server or use a different port with --port",
                    tcp_port
                );
                std::process::exit(1);
            } else {
                ...
                Err(e) => {
                    ...
                    if error_msg.contains("Address already in use") || error_msg.contains("os error 98") {
                        error!(
                            port = tls_port_clone,
                            error = %e,
                            "TLS TCP server failed: Port {} is already in use. Please stop the existing server or use a different port with --tls-port",
                            tls_port_clone
                        );
                        std::process::exit(1);
                    } else {
    ...
    // Start UDP (QUIC) server if UDP port is configured
    if let Some(udp_port) = config.udp_port {
        ...
        tokio::spawn(async move {
            info!(port = udp_port_clone, "Starting UDP (QUIC) server...");
                ...
                Err(e) => {
                    ...
                    if error_msg.contains("Address already in use") || error_msg.contains("os error 98") {
                        error!(
                            port = udp_port_clone,
                            error = %e,
                            "UDP server failed: Port {} is already in use. Please stop the existing server or use a different port with --udp-port",
                            udp_port_clone
                        );
                        std::process::exit(1);
                    } else {
        ...
        info!(port = udp_port, "UDP (QUIC) server enabled");
    }
...
fn print_help() {
    ...
    println!("    -p, --port <PORT>              TCP port to listen on [default: 6784]");
        println!("        --udp-port <PORT>         UDP (QUIC) port to listen on [optional]");
